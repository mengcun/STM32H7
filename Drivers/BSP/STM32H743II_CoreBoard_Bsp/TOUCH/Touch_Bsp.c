...
{ 	 
	...
	Bsp_Delay_us(6);//ADS7846的转换时间最长为6us
	TCLK(0); 	     	    
	Bsp_Delay_us(1);    	   
	TCLK(1);		//给1个时钟，清除BUSY
	Bsp_Delay_us(1);    
...
void TP_Drow_Touch_Point(uint16_t x,uint16_t y,uint16_t color)
{
	...
	Bsp_LCD_DrawPoint(x+1,y+1);
	Bsp_LCD_DrawPoint(x-1,y+1);
	Bsp_LCD_DrawPoint(x+1,y-1);
	Bsp_LCD_DrawPoint(x-1,y-1);
	Bsp_LCD_Draw_Circle(x,y,6);//画中心圈
}	  
//画一个大点(2*2的点)		   
//x,y:坐标
//color:颜色
...
{	    
	POINT_COLOR=color;
	Bsp_LCD_DrawPoint(x,y);//中心点 
	Bsp_LCD_DrawPoint(x+1,y);
	Bsp_LCD_DrawPoint(x,y+1);
	Bsp_LCD_DrawPoint(x+1,y+1);	 	  	
}							  
//////////////////////////////////////////////////////////////////////////////////		  
//触摸按键扫描
...
//提示字符串
uint8_t* const TP_REMIND_MSG_TBL = "Please use the stylus click the cross on the screen.The cross will always move until the screen adjustment is completed.This will exit after 5 seconds.";
...
{	  
	POINT_COLOR=RED;
	Bsp_LCD_ShowString(40,160,lcddev.width,lcddev.height,16,"x1:");
 	Bsp_LCD_ShowString(40+80,160,lcddev.width,lcddev.height,16,"y1:");
 	Bsp_LCD_ShowString(40,180,lcddev.width,lcddev.height,16,"x2:");
 	Bsp_LCD_ShowString(40+80,180,lcddev.width,lcddev.height,16,"y2:");
	Bsp_LCD_ShowString(40,200,lcddev.width,lcddev.height,16,"x3:");
 	Bsp_LCD_ShowString(40+80,200,lcddev.width,lcddev.height,16,"y3:");
	Bsp_LCD_ShowString(40,220,lcddev.width,lcddev.height,16,"x4:");
 	Bsp_LCD_ShowString(40+80,220,lcddev.width,lcddev.height,16,"y4:");  
 	Bsp_LCD_ShowString(40,240,lcddev.width,lcddev.height,16,"fac is:");     
	...
	Bsp_LCD_ShowNum(40+24+80,220,y3,4,16);	//显示数值
 	Bsp_LCD_ShowNum(40+56,240,fac,3,16); 	//显示数值,该数值必须在95~105范围之内.
...
{								 
	...
	Bsp_LCD_Clear(WHITE);//清屏 	   
	POINT_COLOR=BLACK;
	Bsp_LCD_ShowString(10,300,1000,500,24,(uint8_t*)TP_REMIND_MSG_TBL);//显示提示信息
				...
				case 4:	 //全部四个点已经得到
					...
					if(abs(tp_dev.xfac)>2||abs(tp_dev.yfac)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
						Bsp_LCD_ShowString(40,26,lcddev.width,lcddev.height,16,"TP Need readjust!");
					...
					POINT_COLOR=BLUE;
					Bsp_LCD_Clear(WHITE);//清屏
					Bsp_LCD_ShowString(35,110,lcddev.width,lcddev.height,16,"Touch Screen Adjust OK!");//校正完成
					Bsp_Delay_ms(1000);
					TP_Save_Adjdata();  
 					Bsp_LCD_Clear(WHITE);//清屏   
					return;//校正完成				 
