...
{ 	  
	...
	lcddev.id=Bsp_LTDC_PanelID_Read();	//检查是否有RGB屏接入
	if(lcddev.id!=0)
	{
		Bsp_RGB_LCD_Init();			    //ID非零,说明有RGB屏接入.
	}
	else
	{    
        ...
        Bsp_Lcd_Mpu_Config();                       //使能MPU保护LCD区域
        ...
        lcddev.id=Bsp_Lcd_RD_DATA();	//读到0X00
        lcddev.id=Bsp_Lcd_RD_DATA();   	//读取93								   
        lcddev.id<<=8;
        lcddev.id|=Bsp_Lcd_RD_DATA();  	//读取41 	   			   
        if(lcddev.id!=0X9341)		//非9341,尝试看看是不是NT35310
        {	 
            Bsp_Lcd_WR_REG(0XD4);				   
            lcddev.id=Bsp_Lcd_RD_DATA();//dummy read  
            lcddev.id=Bsp_Lcd_RD_DATA();//读回0X01	 
            lcddev.id=Bsp_Lcd_RD_DATA();//读回0X53	
            lcddev.id<<=8;	 
            lcddev.id|=Bsp_Lcd_RD_DATA();	//这里读回0X10	 
            if(lcddev.id!=0X5310)		//也不是NT35310,尝试看看是不是NT35510
            {
                Bsp_Lcd_WR_REG(0XDA00);	
                lcddev.id=Bsp_Lcd_RD_DATA();		//读回0X00	 
                Bsp_Lcd_WR_REG(0XDB00);	
                lcddev.id=Bsp_Lcd_RD_DATA();		//读回0X80
                lcddev.id<<=8;	
                ...
                if(lcddev.id!=0X5510)			//也不是NT5510,尝试看看是不是SSD1963
                {
                    ...
                    lcddev.id=Bsp_Lcd_RD_DATA();	//读回0X57
                    lcddev.id<<=8;	 
                    lcddev.id|=Bsp_Lcd_RD_DATA();	//读回0X61	
        ...
        }else if(lcddev.id==0X1963)
        {
            ...
            Bsp_Lcd_WR_DATA(0x1D);		//参数1 
            Bsp_Lcd_WR_DATA(0x02);		//参数2 Divider M = 2, PLL = 300/(M+1) = 100MHz
            Bsp_Lcd_WR_DATA(0x04);		//参数3 Validate M and N values   
            Bsp_Delay_us(100);
            ...
            Bsp_Delay_ms(12);  
            Bsp_Lcd_WR_REG(0x01);		//软复位
            Bsp_Delay_ms(10);
            
            Bsp_Lcd_WR_REG(0xE6);		//设置像素频率,33Mhz
            Bsp_Lcd_WR_DATA(0x2F);
            Bsp_Lcd_WR_DATA(0xFF);
            Bsp_Lcd_WR_DATA(0xFF);
		
            Bsp_Lcd_WR_REG(0xB0);		//设置LCD模式
            Bsp_Lcd_WR_DATA(0x20);		//24位模式
            Bsp_Lcd_WR_DATA(0x00);		//TFT 模式 
        
            Bsp_Lcd_WR_DATA((SSD_HOR_RESOLUTION-1)>>8);//设置LCD水平像素
            Bsp_Lcd_WR_DATA(SSD_HOR_RESOLUTION-1);		 
            Bsp_Lcd_WR_DATA((SSD_VER_RESOLUTION-1)>>8);//设置LCD垂直像素
            Bsp_Lcd_WR_DATA(SSD_VER_RESOLUTION-1);		 
            Bsp_Lcd_WR_DATA(0x00);		//RGB序列 
            ...
            Bsp_Lcd_WR_REG(0xF0);	//设置SSD1963与CPU接口为16bit  
            Bsp_Lcd_WR_DATA(0x03);	//16-bit(565 format) data for 16bpp 

            Bsp_Lcd_WR_REG(0x29);	//开启显示
            //设置PWM输出  背光通过占空比可调 
            Bsp_Lcd_WR_REG(0xD0);	//设置自动白平衡DBC
            Bsp_Lcd_WR_DATA(0x00);	//disable
        
            Bsp_Lcd_WR_REG(0xBE);	//配置PWM输出
            Bsp_Lcd_WR_DATA(0x05);	//1设置PWM频率
            Bsp_Lcd_WR_DATA(0xFE);	//2设置PWM占空比
            Bsp_Lcd_WR_DATA(0x01);	//3设置C
            Bsp_Lcd_WR_DATA(0x00);	//4设置D
            Bsp_Lcd_WR_DATA(0x00);	//5设置E 
            Bsp_Lcd_WR_DATA(0x00);	//6设置F 
            
            Bsp_Lcd_WR_REG(0xB8);	//设置GPIO配置
            Bsp_Lcd_WR_DATA(0x03);	//2个IO口设置成输出
            Bsp_Lcd_WR_DATA(0x01);	//GPIO使用正常的IO功能 
            Bsp_Lcd_WR_REG(0xBA);
            Bsp_Lcd_WR_DATA(0X01);	//GPIO[1:0]=01,控制LCD方向
            
            Bsp_LCD_SSD_BackLightSet(100);//背光设置为最亮
        }	    
    }	
	//初始化完成以后,提速
...
{
	uint16_t lcdid=0;
	lcdid=Bsp_LTDC_PanelID_Read();			//读取LCD面板ID	
	if(lcdid==0X4342)
	{
		...
		lcdltdc.hfp=5;				    //水平前廊
		lcdltdc.vfp=8;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,88);			//设置像素时钟 9Mhz 
		//其他参数待定.
	}
	else if(lcdid==0X7084)
	{
		...
		lcdltdc.vfp=22;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,24);			//设置像素时钟 33M(如果开双显,需要降低DCLK到:18.75Mhz  pll3r=43,才会比较好)
	}
	else if(lcdid==0X7016)		
	{
		...
		lcdltdc.vfp=12;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,18);			//设置像素时钟  45Mhz 
		//其他参数待定.
	}
	...
	else if(lcdid==0X1018)				//10.1寸1280*800 RGB屏	
	{
		...
		lcdltdc.vfp=10;					//垂直前廊
		lcdltdc.vsw=3;					//垂直同步宽度
		Bsp_LTDC_Clk_Set(5,160,16);			//设置像素时钟 50MHz
	}	
	...
	{ 
		...
		lcdltdc.vbp=24;				    //垂直后廊  
		lcdltdc.vfp=3;				    //垂直前廊
        Bsp_LTDC_Clk_Set(5,160,28);//设置像素时钟 28.5Mhz
    ...
    {  
		...
		lcdltdc.vbp=14;				    //垂直后廊 
		lcdltdc.vfp=3;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,22);//设置像素时钟 35.5Mhz
	}
	else if(lcdid==0XA003)	        //接VGA显示器
	{
		...
		lcdltdc.vbp=12;				    //垂直后廊
		lcdltdc.vfp=3;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,23);//设置像素时钟  34.25Mhz
	}
	else if(lcdid==0XA004)            //接VGA显示器
	{
		...
		lcdltdc.vbp=29;				    //垂直后廊 
		lcdltdc.vfp=3;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,18);//设置像素时钟 43.25Mhz	
	}
	else if(lcdid==0XA005)            //接VGA显示器
	{
		...
		lcdltdc.vbp=23;				    //垂直后廊
		lcdltdc.vfp=6;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,47);//设置像素时钟 17Mhz 
	}
	else if(lcdid==0XA006)            //接VGA显示器
	{
		...
		lcdltdc.vbp=23;				    //垂直后廊
		lcdltdc.vfp=1;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,40);//设置像素时钟 20mz		
	}
	else if(lcdid==0XA007)            //接VGA显示器
	{
		...
		lcdltdc.vbp=41;				    //垂直后廊
		lcdltdc.vfp=2;				    //垂直前廊
		Bsp_LTDC_Clk_Set(5,160,62);//设置像素时钟 12.75Mhz
	} 
	...
	//层配置
	Bsp_LTDC_Layer_Parameter_Config(0,(uint32_t)ltdc_framebuf[0],LCD_PIXFORMAT,255,0,6,7,0X000000);//层参数配置
	Bsp_LTDC_Layer_Window_Config(0,0,0,lcdltdc.pwidth,lcdltdc.pheight);	//层窗口配置,以LCD面板坐标系为基准,不要随便修改!	
	 	
 	Bsp_LTDC_Display_Dir(1);			//默认竖屏
	Bsp_LTDC_Select_Layer(0); 			//选择第1层
	LCD_LED(1);         		    //点亮背光
	Bsp_LTDC_Clear(0XFFFFFFFF);			//清屏
}
//LTDC底层IO初始化和时钟使能
...
//注意:此函数必须在Bsp_LTDC_Layer_Parameter_Config之后再设置.
//layerx:层值,0/1.
//sx,sy:起始坐标
//width,height:宽度和高度
...
//LTDC,基本参数设置.
//注意:此函数,必须在Bsp_LTDC_Layer_Window_Config之前设置.
//layerx:层值,0/1.
//bufaddr:层颜色帧缓存起始地址
...
{										   
	Bsp_Lcd_WR_REG(LCD_Reg);		//写入要读的寄存器序号
	Bsp_Delay_us(5);		  
	return Bsp_Lcd_RD_DATA();		//返回读到的值
}   
//开始写GRAM
...
uint32_t Bsp_LCD_ReadPoint(uint16_t x,uint16_t y)
{
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)Bsp_Lcd_WR_REG(0X2E);//9341/3510/1963 发送读GRAM指令
	else if(lcddev.id==0X5510)Bsp_Lcd_WR_REG(0X2E00);	//5510 发送读GRAM指令
...
{					   
	if(lcdltdc.pwidth!=0)Bsp_LTDC_Switch(1);//开启LCD
	else if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)Bsp_Lcd_WR_REG(0X29);	//开启显示
	else if(lcddev.id==0X5510)Bsp_Lcd_WR_REG(0X2900);	//开启显示
}	 
//LCD关闭显示
void Bsp_LCD_DisplayOff(void)
{	   
	if(lcdltdc.pwidth!=0)Bsp_LTDC_Switch(0);//关闭LCD
	else if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)Bsp_Lcd_WR_REG(0X28);	//关闭显示
	else if(lcddev.id==0X5510)Bsp_Lcd_WR_REG(0X2800);	//关闭显示
}   
//设置光标位置(对RGB屏无效)
//Xpos:横坐标
//Ypos:纵坐标
...
{	
	Bsp_Lcd_WR_REG(0xBE);	//配置PWM输出
	Bsp_Lcd_WR_DATA(0x05);	//1设置PWM频率
	Bsp_Lcd_WR_DATA(pwm*2.55);//2设置PWM占空比
	Bsp_Lcd_WR_DATA(0x01);	//3设置C
	Bsp_Lcd_WR_DATA(0xFF);	//4设置D
	Bsp_Lcd_WR_DATA(0x00);	//5设置E
	Bsp_Lcd_WR_DATA(0x00);	//6设置F
}
...
{  
	...
	else
	{
		...
		for(i=0;i<height;i++)
		{
			Bsp_LCD_SetCursor(sx,sy+i);   	//设置光标位置 
			Bsp_LCD_WriteRAM_Prepare();     //开始写入GRAM
